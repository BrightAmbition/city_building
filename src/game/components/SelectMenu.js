class SelectMenu {
  constructor(scene, bg_down, bg_up, bg_item, x = 0, y = 0) {
    this.scene = scene;
    this.x = x;
    this.y = y;
    this.items = [
    ...
    // Infomations
    const infos = this.scene.add.container(x, y).setAlpha(0).setDepth(2);

    for (let i = 0; i < this.items.length; i++) {
      const info_item = this.scene.add
        .image(0, i * 50 + 40, bg_item)
        .setOrigin(0, 0)
      ...
      const info_item_txt = this.scene.add.text(
        20,
        10 + i * 50 + 40,
        this.items[i],
        {
      ...
      infos.add([info_item, info_item_txt, info_item_tile]);
    }
